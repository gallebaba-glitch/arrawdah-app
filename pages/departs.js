...
export default function Departs() {
  ...
  return (
    <Layout title={`Départs (${departs.length})`}
      ...
      const ph = c.pelerins.map(p =>
        '<div class="pelerin">' + p.prenom + ' ' + p.nom +
        ' <span class="badge ' + (p.sexe === 'femme' ? 'badge-f' : 'badge-h') + '">' + (p.sexe === 'femme' ? 'F' : 'H') + '</span></div>'
      ).join('')
      return '<div class="ch"><div class="ch-h" style="background-color:' + colorCh(c) + ';">Chambre ' + c.numero + ' — ' + labelCh(c) + ' · ' + c.formule + ' (' + c.pelerins.length + '/' + c.cap + ')</div><div class="ch-b">' + ph + '<div class="hotel-row"><span class="hotel-label">N° hôtel :</span><input class="hotel-input" type="text" inputmode="numeric" placeholder="ex: 412" /></div></div></div>'
    }).join('')}
    </div>useState } from 'react'
import Layout from '../components/Layout'
import Modal from '../components/Modal'
import { supabase } from '../lib/supabase'
