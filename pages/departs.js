...
// ─── GÉNÉRATION AUTOMATIQUE CHAMBRES ─────────────
function genererChambres(pelerins){
  const g={ZEN_homme:[],ZEN_femme:[],ELITE_homme:[],ELITE_femme:[]}
  pelerins.forEach(p=>{const k=`${p.formule}_${p.sexe||'homme'}`;if(g[k])g[k].push(p)})
  ...
  Object.entries(g).forEach(([key,liste])=>{
    ...
    for(let i=0;i<liste.length;i+=cap){
      result.push({id:`auto_${num}`,numero:num++,formule,sexe,cap,pelerins:liste.slice(i,i+cap),isAuto:true})
    }
  })
  return result
}
...
export default function Departs(){
  ...
  // État des chambres manuelles (par départ)
  const [chambresData,setChambresData]=useState(null)  // null = auto
  const [moveModal,setMoveModal]=useState(null)  // {pelerin, fromChambre}
  ...
  // Quand on change de départ sélectionné, réinitialiser les chambres
  function selectDepart(id){
    setSelected(id===selected?null:id)
    setChambresData(null)
    setActiveTab('liste')
  }
  ...
  // Chambres actives = manuelles si modifiées, sinon auto
  const chambresAuto=sel?genererChambres(selPelerins):[]
  const chambres=chambresData||chambresAuto

  // ── ACTIONS CHAMBRES ──────────────────────────

  // Initialiser les chambres manuelles depuis l'auto
  function initManuel(){
    if(!chambresData)setChambresData(chambresAuto.map(c=>({...c,pelerins:[...c.pelerins]})))
  }

  // Ajouter une place dans une chambre
  function ajouterPlace(chambreId){
    initManuel()
    setChambresData(prev=>{
      const data=prev||chambresAuto.map(c=>({...c,pelerins:[...c.pelerins]}))
      return data.map(c=>c.id===chambreId?{...c,cap:c.cap+1}:c)
    })
  }

  // Retirer une place (seulement si vide)
  ...
  // Déplacer un pèlerin vers une autre chambre
  function deplacerPelerin(toChambreId){
    if(!moveModal)return
    const{pelerin,fromChambre}=moveModal
    setChambresData(prev=>{
      const data=(prev||chambresAuto.map(c=>({...c,pelerins:[...c.pelerins]}))).map(c=>({...c,pelerins:[...c.pelerins]}))
      // Retirer de la chambre source
      const src=data.find(c=>c.id===fromChambre.id)
      if(src)src.pelerins=src.pelerins.filter(p=>p.id!==pelerin.id)
      // Ajouter dans la chambre cible
      const tgt=data.find(c=>c.id===toChambreId)
      if(tgt){
        if(tgt.pelerins.length>=tgt.cap){
          alert(`La chambre ${tgt.numero} est complète (${tgt.cap} places). Augmentez sa capacité d'abord.`)
          return prev
        }
  ...
  // Retirer un pèlerin d'une chambre (le met en "non assigné")
  ...
  const pelerinsAssignes=chambresData?chambresData.flatMap(c=>c.pelerins.map(p=>p.id)):[]
  const nonAssignes=selPelerins.filter(p=>!pelerinsAssignes.includes(p.id))

  // Ajouter pèlerin non assigné dans une chambre
  function ajouterPelerinDansChambre(pelerin,chambreId){
    ...
    setChambresData(prev=>{
      ...
      const ch=data.find(c=>c.id===chambreId)
      if(ch){
        if(ch.pelerins.length>=ch.cap){alert(`Chambre ${ch.numero} complète. Augmentez la capacité d'abord.`);return prev}
  ...
  // Réinitialiser vers l'auto
  function reinitialiserChambres(){
    if(confirm('Réinitialiser la répartition automatique ? Toutes vos modifications seront perdues.'))
      setChambresData(null)
  }
              ...
              <div className="bg-white rounded-xl border sticky top-20" style={{borderColor:'#E5EDE8'}}>
                ...
                <div className="p-4 rounded-t-xl flex items-center justify-between" style={{background:'#0F5229'}}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={()=>{setSelected(null);setChambresData(null)}} className="text-white/60 hover:text-white ml-2">✕</button>
                  </div>
                </div>
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                          {chambresData?'✏️ Mode manuel':'🔄 Répartition automatique'}
                        </div>
                        {chambresData&&(
                      ...
                      {/* Pèlerins non assignés */}
                      {chambresData&&nonAssignes.length>0&&(
                        <div className="mb-3 p-3 rounded-lg border-2 border-dashed" style={{borderColor:'#FCA5A5',background:'#FFF5F5'}}>
                          <div className="text-xs font-bold text-red-500 mb-2">⚠️ Pèlerins non assignés ({nonAssignes.length})</div>
                          <div className="space-y-1">
                            {nonAssignes.map(p=>(
                              <div key={p.id} className="flex items-center justify-between text-xs p-1.5 bg-white rounded border" style={{borderColor:'#FECACA'}}>
                                ...
                                <select onChange={e=>{if(e.target.value)ajouterPelerinDansChambre(p,e.target.value)}}
                                  className="text-xs border rounded px-1 py-0.5 ml-2" style={{borderColor:'#E5EDE8'}}
                                  defaultValue="">
                        ...
                        <div key={ch.id} className="border rounded-lg overflow-hidden mb-3" style={{borderColor:'#E5EDE8'}}>
                          ...
                          <div className="flex items-center justify-between px-3 py-2"
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs font-semibold px-2 py-0.5 rounded-full"
                                style={{background:ch.pelerins.length>=ch.cap?'#FEE2E2':'#E8F5EE',
                                        color:ch.pelerins.length>=ch.cap?'#991B1B':'#0F5229'}}>
                          ...
                          <div className="divide-y" style={{borderColor:'#F0F0F0'}}>
                            {ch.pelerins.map(p=>(
                              <div key={p.id} className="flex items-center gap-2 px-3 py-2">
                                ...
                                <button onClick={()=>ouvrirDeplacement(p,ch)}
                                  ...
                                  style={{background:'#EFF8FF',color:'#1D4ED8'}}
                                  title="Déplacer dans une autre chambre">
                                  ↔️
                                </button>
                                {/* Bouton retirer */}
                            ...
                            {Array.from({length:ch.cap-ch.pelerins.length}).map((_,i)=>(
                              <div key={i} className="px-3 py-2 text-xs text-gray-300 italic">
                                — Place disponible
                              </div>
                            ))}
                          </div>
      ...
      {moveModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.5)'}}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="p-4">
              <div className="text-xs font-semibold text-gray-500 uppercase mb-2">Choisir la chambre de destination</div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-sm font-semibold">
                          Chambre {c.numero} — {c.sexe==='femme'?'👩 Femmes':'👨 Hommes'} · {c.formule}
                        </div>
                      ...
                      <span className="text-xs font-bold ml-2 px-2 py-0.5 rounded-full"
                        style={{background:c.pelerins.length>=c.cap?'#FEE2E2':'#E8F5EE',
                                color:c.pelerins.length>=c.cap?'#991B1B':'#0F5229'}}>
            ...
            <div className="p-4 border-t" style={{borderColor:'#E5EDE8'}}>
              <button onClick={()=>setMoveModal(null)}
                ...
                style={{borderColor:'#E5EDE8',color:'#6B7280'}}>
                Annuler
              </button>
            </div>
          </div>
